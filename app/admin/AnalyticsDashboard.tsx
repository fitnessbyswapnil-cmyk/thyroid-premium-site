...
/**
 * The ANALYTICS half of /admin — funnel, ROAS, ad quality, follow-up queue.
 * Rendered as one tab of app/admin/page.tsx; the other is the Pipeline.
 *
 * Was itself /admin/page.tsx until the pipeline shipped. Moved verbatim rather
 * than rewritten: every number in here is load-bearing and well-tested by use.
 *
...
function buildMessage(l: Lead): { kind: string; text: string } | null {
  ...
  // AN UNPAID WOMAN ALWAYS GETS THE FIRST-TOUCH DRAFT, whatever her booking
  // history says. Every branch below assumes money already changed hands:
  // "your slot is already paid for", "your Rs 299 slot is still reserved",
  // "great speaking with you today". Sent to someone who booked through the
  // old ungated page and never paid — or who no-showed a call she never paid
  // for — they promise a slot that does not exist and skip straight past the
  // reason she never bought. The draft restarts the conversation from her own
  // form instead, which is the only ground she has actually given us.
  if (!l.paid && l.draftMessage) {
  ...
  // Fallback only, for rows the server could not draft for — a lead whose quiz
  // columns are empty, so there is nothing personal to open with.
  return {
    kind: "Nudge",
...
export default function AnalyticsDashboard() {
  ...
  const ops = useMemo(() => {
    ...
    //
    // TWO REVENUE LINES, NEVER ONE. The Rs 299 consultation and the programme
    // that follows it behave nothing alike: the first is a near-automatic
    // trickle that only ever repays a slice of the ad bill, the second is where
    // the business actually is. Summing them into a single "revenue" figure
