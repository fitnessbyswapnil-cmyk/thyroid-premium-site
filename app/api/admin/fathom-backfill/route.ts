...
async function backfill(limit: number, force: boolean) {
  ...
  for (const m of meetings) {
    ...
    try {
      ...
      if (plan.action !== "skip") await fireCallHeld({ uid: match.uid, name: fields.name ?? "", email: fields.email ?? "", phone: fields.phone ?? "", attended: x.attended, emails: m.emails, startedAt: m.startedAt });
      results.push({
